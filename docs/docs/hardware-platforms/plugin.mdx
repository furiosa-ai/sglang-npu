...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      <td><code>validate_server_args(server_args)</code></td>
      <td>After all ServerArgs declarations are resolved</td>
      <td>Validate platform-specific requirements</td>
    </tr>
    <tr>
...
The two `ServerArgs` hooks run during normal initialization. The `none` and
`dummy` model paths return before either hook. `validate_server_args` receives
the final materialized values and must not modify them.
